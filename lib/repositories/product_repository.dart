...
class ProductRepository {
  ...
  Future<List<Product>> getHomeCategoryProducts({@required String url}) async {
    Uri _url = Uri.parse("$url");

    print(_url);

    return await http.get(_url, headers: {
      "App-Language": app_language.$,
    }).then((_response) {
      final ProductMiniResponse _productsResponse =
          productMiniResponseFromJson(_response.body);
      if (_productsResponse.success) {
        return _productsResponse.products;
      } else {
        return [];
      }
    });
